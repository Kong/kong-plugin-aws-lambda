...
local create_ec2_provider = function ()
  --
  -- PRIVATE FUNCTIONS
  --
  ...
  local function fetch_ec2_credentials()
    ...
    local connect_metadata_res, connect_metadata_err = client:connect(METADATA_SERVICE_HOST, METADATA_SERVICE_PORT)
    if not connect_metadata_res then
      return nil, "Could not connect to metadata service: " .. tostring(connect_metadata_err)
    end

    local role_name_request_res, get_role_name_err = client:request {
      method = "GET",
      path   = "/latest/meta-data/iam/security-credentials/",
    }

    if not role_name_request_res then
      return nil, "Could not fetch role name from metadata service: " .. tostring(get_role_name_err)
    end
    ...
    log.debug("Found IAM role on instance with name: ", iam_role_name)
    ...
    log.debug("Received temporary IAM credential from metadata service for role '",
...
return {
  ...
  get_provider = create_ec2_provider
}
