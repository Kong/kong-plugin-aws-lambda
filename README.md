...
|`config.aws_assume_role_arn` <br>*semi-optional* || The assume role is used to assume a role different than the role defined by the variable AWS_ROLE_ARN in the EKS environment. This value is used if the value `aws_key` is not defined. This value offers the possibility to use a cross account role.
